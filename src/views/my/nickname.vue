<template>
  <div class="container">
    ...
    <!-- 输入框 -->
    <inputBox :config="inputConfig" ref="headerChild"></inputBox>
    <!-- 按钮 -->
...
export default {
  name: 'personaldata',
  components: { Head, inputBox, Buttom },
  props: ['config'],
  data() {
    return {
      wrapConfig: {
      ...
      conf: {
        title: '确定',
        handelClick: () => {
          let name1 = this.$refs.headerChild.name;
          this.$router.push({ name: 'personaldata', query: { name1 } });
